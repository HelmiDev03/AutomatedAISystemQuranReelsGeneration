"""
Standalone Hadith Generator

Generates a Hadith quote card end-to-end and saves the output metadata 
to `latest_hadith_generation.json` to be picked up by the publisher script.
"""
...
from hadith_generator.generate_post import generate as generate_hadith_image
...
async def generate():
    ...
    # Load recent topics
    recent_topics_file = "latest_hadith_topics.json"
    recent_topics = []
    ...
    try:
        ...
        prompt = (
            "the hadith must be in only arabic text. Do not answer in any other language."
            f"Choose a unique, beautiful, and inspirational Islamic topic suitable for a Hadith post. "
            ...
            "Give me only the core text (Matn) of the Hadith. Do not include the narrator chain, source, or any introductory phrases like 'The Prophet said'.\n"
    ...
    # 4. Image Rendering
    print_header("Rendering Hadith Image")
    try:
        image_path = generate_hadith_image(arabic)
        print_ok(f"Hadith image card saved to: {image_path}")
    except Exception as e:
        print_fail(f"Hadith image rendering failed: {e}")
        sys.exit(1)
    ...
    with open("latest_hadith_generation.json", "w", encoding="utf-8") as f:
        json.dump(output_data, f, ensure_ascii=False, indent=2)
        
    print_ok("Saved latest_hadith_generation.json")
    ...
    print_ok("Generation Pipeline Complete! Run publish_hadith.py next.")
